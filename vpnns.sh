...
Create and configure network namespace for isolated environment connected via OpenVPN to remote network.\n
  OPTIONS:
    ...
    --help          hmmmmm...
    --ns-up         create network namespace with name ${COLORYELLOW}${VPNNS}${COLORNONE}; name can be changed in script body, no other customizations so far
    --ns-down       delete network namespace with name ${COLORYELLOW}${VPNNS}${COLORNONE}; also terminates all applications that uses this namespace (careful!)
...
VPN configuration should have name \"${COLORYELLOW}${VPNCFG}${COLORNONE}\"; set it using variable VPNNAME.
Configuration will be searched in \"${COLORYELLOW}/home/$USER/openvpn${COLORNONE}\" and then in \"${SYSTEM_VPNCFGDIR}\". Can be overridden by variable VPNCFGDIR
